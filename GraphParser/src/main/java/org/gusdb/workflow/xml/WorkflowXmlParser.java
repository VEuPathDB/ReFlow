...
/**
 * Parses an XML workflow specification into a usable workflow.  The constructor takes a class factory that
 * determines which classes are generated by this parser.
 * 
 * @author rdoherty
 *
 * @param <S> The type of workflow node to generate
 * @param <T> The type of container for the workflow nodes to generate
 */
public class WorkflowXmlParser<S extends WorkflowNode, T extends WorkflowXmlContainer<S>> extends XmlParser {
  ...
  public WorkflowXmlParser(WorkflowClassFactory<S,T> classFactory) {
    _digester = configureDigester(classFactory);
  }

  public T parseWorkflow(String xmlFileName, String callerXmlFileName)
      throws SAXException, IOException, ParserConfigurationException, TransformerException {
    return parseWorkflow(xmlFileName, callerXmlFileName, true);
  }

  public T parseWorkflow(String xmlFileName, String callerXmlFileName, boolean useGusHome)
      throws SAXException, IOException, ParserConfigurationException, TransformerException {
  ...
  private T parseXml(Document doc, Map<String, String> properties) throws TransformerException, IOException,
      SAXException {
    InputStream xmlStream = substituteProps(doc, properties);
    return (T) _digester.parse(xmlStream);
  }

  private Digester configureDigester(WorkflowClassFactory<S,T> classFactory) {
    Digester digester = new Digester();
    digester.setValidating(false);

    // Root -- WDK Model
    digester.addObjectCreate("workflowGraph", classFactory.getContainerClass());
    ...
    configureNode(digester, "workflowGraph/step", classFactory.getStepClass(), "addStep");
    ...
    configureNode(digester, "workflowGraph/subgraph", classFactory.getStepClass(), "addStep");
    ...
    configureNode(digester, "workflowGraph/globalSubgraph", classFactory.getStepClass(), "addGlobalStep");
