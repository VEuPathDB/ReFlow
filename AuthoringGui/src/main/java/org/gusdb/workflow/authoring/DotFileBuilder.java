...
public class DotFileBuilder {
  ...
  public static void main(String[] args) {
    try {
      ...
      StringBuilder output = new StringBuilder().append("digraph name {").append(NL).append(
          "  graph [ fontsize=8, rankdir=\"TB\" ]").append(NL).append(
          "  node [ fontsize=8, height=0, width=0, margin=0.03,0.02 ]").append(NL).append(
          "  edge [ fontsize=8, arrowhead=open ]").append(NL);
      ...
      for (String groupName : groupingMap.keySet()) {
        ...
        output.append("subgraph ").append(clusterName).append(" {").append(NL).append("  label = \"").append(
            getValidGroupName(groupName)).append("\"").append(NL);
      ...
      for (SimpleXmlNode vertex : graph.getNodes()) {
        ...
        for (Name dependency : vertex.getDependsNames()) {
          output.append("  ").append(formatNodeLabel(dependency.getName())).append(" -> ").append(nodeLabel).append(
              NL);
        }
      }
  ...
  private static void dumpVertexInfo(StringBuilder output, List<SimpleXmlNode> nodeList) {
    for (SimpleXmlNode vertex : nodeList) {
      String nodeLabel = formatNodeLabel(vertex.getBaseName());
      output.append("  ").append(nodeLabel).append(" [ shape=rectangle");
  ...
  /**
   * Base name passed in is a camel-case string representing a name; it may also contain underscores and
   * dashes, which must be trimmed and/or formatted nicely. The first letter should also be capitalized if it
   * isn't already.
   * 
   * @param baseName
  ...
  private static void printUsageAndDie() {
    System.err.println("USAGE: java " + DotFileBuilder.class.getName() +
        " [--backtrack <relativePath>] <workflowXmlFile>");
    System.exit(1);
  }
}
