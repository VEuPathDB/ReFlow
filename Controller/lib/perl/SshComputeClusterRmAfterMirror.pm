...
sub copyFrom {
 ...
 #   $self->{mgr}->runCmd(0, "ssh -2 $ssh_target 'cd $fromDir; tar cf - $fromFile | gzip -c | tee (md5sum sum)' | tee (md5sum sum) | gunzip -c | tar xf -");
 #   my $checksumOnCluster = $self->{mgr}->runCmd(0, "ssh -2 $ssh_target 'cd $fromDir; cat sum");
 #   my $checksumLocal = $self->{mgr}->runCmd(0, "cat sum");
 #   $self->{mgr}->error("Checksum on cluster '$checksumOnCluster' and checksum locally '$checksumLocal' do not match.  It appears the copy from cluster of file '$fromDir/$fromFile' failed") unless $checksumOnCluster eq $checksumLocal;
